...
export default function ProductTable() {
  ...
  const handleAdd = () => { setSelectedProduct(null); setIsFormOpen(true); };
  const handleEdit = (product: Product) => { setSelectedProduct(product); setIsFormOpen(true); };
  const handleDelete = (product: Product) => { setSelectedProduct(product); setIsDeleteDialogOpen(true); };
  ...
  const columns = useMemo(() => getProductColumns({ onEdit: handleEdit, onDelete: handleDelete }), []);
  ...
  return (
    <div>
      <div className="flex flex-col md:flex-row items-center justify-between mb-4 bg-neutral-50 p-4 rounded-md shadow-sm">
        <div>
          <h1 className="text-3xl font-bold">Manajemen <span className="text-primary">Produk</span></h1>
          <p className="text-muted-foreground pt-1 text-sm text-center md:text-left">Kelola semua produk MadeByCan</p>
        </div>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent className="sm:max-w-4xl md:max-w-7xl max-h-screen overflow-y-auto flex-col justify-center">
          <DialogHeader>
            <DialogTitle>{selectedProduct ? "Edit Produk & Varian" : "Tambah Produk Baru"}</DialogTitle>
             <DialogDescription>
                Isi detail produk di bawah ini. Anda bisa menambahkan beberapa varian dengan harga dan stok yang berbeda.
            </DialogDescription>
          </DialogHeader>
          <ProductForm
