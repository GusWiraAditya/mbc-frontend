    ...
    <Card className="bg-emerald-50 border-emerald-200 overflow-hidden">
      <CardHeader className="p-4 flex flex-row items-start justify-between">
        <div className="space-y-1">
          <CardTitle className="text-md text-emerald-900 flex items-center gap-2">
            <Tag className="h-5 w-5" /> {voucher.code}
          </CardTitle>
          <CardDescription className="text-emerald-700 text-sm font-medium">{voucher.name}</CardDescription>
        </div>
        <Button variant="ghost" size="icon" className="h-7 w-7 text-emerald-700 hover:bg-emerald-100 shrink-0" onClick={() => removeVoucher(voucher.code)} disabled={isApplyingVoucher}>
          <X className="h-4 w-4" />
        </Button>
      </CardHeader>
      <CardContent className="p-4 pt-0 text-xs text-emerald-800 space-y-2">
        {voucher.description && (
        ...
        <div className="flex items-start gap-2">
          ...
          <p>Berlaku: {formattedStartDate} - {formattedEndDate}</p>
        </div>
      </CardContent>
    </Card>
  );
...
const VoucherDialog = () => {
  ...
  const handleApply = () => {
    ...
    applyVoucher(inputCode.trim()).then(() => {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogTrigger asChild>
        <Button variant="outline" className="w-full justify-start text-primary hover:text-primary hover:bg-primary/5 border-primary/30">
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>Masukkan kode voucher atau lihat voucher yang sedang Anda gunakan.</DialogDescription>
        </DialogHeader>
        
        <div className="space-y-2">
          <Label htmlFor="voucher" className="sr-only">Tambah Voucher Baru</Label>
          <div className="flex space-x-2">
            ...
            <Button onClick={handleApply} disabled={!inputCode || isApplyingVoucher}>
...
const CartItemCard = ({ item }: { item: CartItem }) => {
  const { updateQuantity, removeFromCart, toggleSelectItem } = useCartStore();

  // Gunakan useMemo untuk mencegah kalkulasi ulang yang tidak perlu
  const formattedPrice = useMemo(() => new Intl.NumberFormat("id-ID", {
    style: "currency", currency: "IDR", minimumFractionDigits: 0,
  }).format(item.price), [item.price]);

  return (
    <div className="flex items-start space-x-4 py-4 justify-center">
      <Checkbox
        className="mt-5"
      ...
      <div className="relative w-24 h-24 rounded-md overflow-hidden bg-gray-100">
        <Image
          src={item.image ? `${process.env.NEXT_PUBLIC_LARAVEL_API_URL}/storage/${item.image}` : "/placeholder.png"}
      ...
      <div className="flex flex-col items-end justify-between h-24">
         <p className="text-lg font-bold text-primary mt-1">{formattedPrice}</p>
          <div className="flex justify-between items-center space-x-2">
            <Button variant="ghost" size="icon" className="text-gray-400 hover:text-red-600 hover:bg-red-50" onClick={() => removeFromCart([item.variantId])}>
    ...
    <div className="bg-white rounded-lg shadow-sm p-6 sticky top-24">
      ...
      <div className="space-y-4 my-4">
        <div className="flex justify-between">
          <span className="text-gray-600">Subtotal ({selectedItemsCount} produk)</span>
          <span className="font-semibold text-gray-800">{formatCurrency(summary.subtotal)}</span>
        </div>
      ...
      <Button size="lg" className="w-full" disabled={selectedItemsCount === 0} onClick={() => router.push('/checkout')}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 lg:gap-8 items-start">
          ...
          <div className="lg:col-span-2 bg-white rounded-lg shadow-sm p-6 relative">
            ...
            <div className="flex justify-between items-center border-b pb-4 mb-4">
              ...
              <Button variant="ghost" size="sm" className="text-red-600 hover:text-red-700 hover:bg-red-50" onClick={() => removeFromCart(selectedItems.map((item) => item.variantId))} disabled={selectedItems.length === 0 || isLoading}>
