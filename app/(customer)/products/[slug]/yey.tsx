  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const relatedRes = await api.get(`/api/products/${slug}/related`);
      ...
      <div className="container mx-auto px-4 py-10">
        ...
        <Breadcrumb className="mb-8">
          <BreadcrumbList>
            ...
            <BreadcrumbSeparator />
            <BreadcrumbItem><BreadcrumbLink asChild><Link href="/collections">Koleksi</Link></BreadcrumbLink></BreadcrumbItem>
            <BreadcrumbSeparator />
        ...
        <div className="grid md:grid-cols-2 gap-12">
          ...
          <div className="space-y-4">
            <div className="relative w-full aspect-square rounded-lg overflow-hidden border">
              <AnimatePresence mode="wait">
                ...
                >
                  <Image
                    src={selectedVariant?.images[0]?.path ? `${process.env.NEXT_PUBLIC_LARAVEL_API_URL}/storage/${selectedVariant.images[0].path}` : '/placeholder.png'}
            ...
            <div className="grid grid-cols-5 gap-2">
              {selectedVariant?.images.map((image, index) => (
                <div key={image.id} className="relative aspect-square rounded-md overflow-hidden border-2 border-transparent">
          ...
          <div className="space-y-6">
            ...
            <div>
              <h3 className="text-md font-semibold mb-3">Warna: <span className="font-normal text-muted-foreground">{selectedVariant?.color.name}</span></h3>
            ...
            <div>
              <h3 className="text-md font-semibold mb-2">Ukuran:</h3>
            ...
            <div className="flex items-center gap-4 pt-2">
               <Button size="lg" disabled={!selectedVariant || selectedVariant.stock === 0} className="w-full md:w-auto">
                  {selectedVariant && selectedVariant.stock > 0 ? 'Tambah ke Keranjang' : 'Stok Habis'}
               </Button>
               <div className="text-sm text-muted-foreground">Stok: {selectedVariant?.stock || 0}</div>
            </div>
            ...
            <Accordion type="multiple" className="w-full" defaultValue={['item-1']}>
                ...
                <AccordionContent className="text-muted-foreground prose prose-sm max-w-none space-y-4">
                  ...
                  {product.category.description && (
                    <div className="text-xs border-t pt-2 mt-4">
                        <p className="font-semibold text-gray-600">Tentang Kategori {product.category.category_name}:</p>
              ...
              {selectedVariant?.material.description && (
                <AccordionItem value="item-2">
                  <AccordionTrigger>Detail Material</AccordionTrigger>
      ...
      {relatedProducts.length > 0 && (
         <section className="bg-muted py-16">
            <div className="container mx-auto px-4">
              <h2 className="text-2xl font-bold text-center mb-8">Anda Mungkin Juga Suka</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
                {relatedProducts.map(related => {
                  const displayPrice = related.min_price && related.max_price && related.min_price > 0 
                    ? (related.min_price === related.max_price ? new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(related.min_price) : `Mulai dari ${new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", minimumFractionDigits: 0 }).format(related.min_price)}`)
                    : 'Harga bervariasi';
